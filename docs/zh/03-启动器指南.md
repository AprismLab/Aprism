...
JE 版本按 Mojang 配置映射到所需的 Java 运行时。启动器维护一张表，将每个主要 Minecraft 版本映射到其推荐的 Java 主版本：
...
| 1.21+ | 21 | Aprism 最低运行时 |
| 1.21.6+（计划中） | 25 | Aprism 前向兼容 |

Aprism 自身要求 Java 21 作为启动器进程的基线运行时。agent jar 以 `--release 21` 字节码构建，并对 Java 25 前向兼容。如果在用户的 PATH 或已配置的运行时列表中未检测到匹配的运行时，启动器应提议安装 Mojang 提供的 Java 运行时（来自版本 JSON 中的 `javaRuntimeManifest`）。

### 3.4 Mod 目录管理

JE mod 存放于 `.minecraft/mods/`（扁平目录，或在 MultiMC/Prism 中按 `mods/<version>/` 的每实例布局）。Aprism `.aje` 归档与标准 `.jar` mod 放置在同一目录。Aprism agent 在启动时扫描 `mods/`，并将任何以 `.aje` 扩展名结尾的文件作为 Aprism mod container 加载；`.jar` 文件则原样传递给 Fabric/NeoForge，不做修改。
...
默认禁用嵌套目录扫描，以避免拾取某些启动器用作暂存区的 `mods/<disabled>/` 存放目录。启动器提供一个显式的"Aprism mods 文件夹"视图，列出 `.aje` 文件与共存的 `.jar` mod。
...
该模块从 `/data/adb/aprism/config.json` 读取其配置，并从 `/data/adb/aprism/mods/` 提供 `.abe` mod。到 `/sdcard/games/com.mojang/behavior_packs/` 的符号链接允许世界作用域的包被解析。
...
`server.properties` 集成：启动器将 Aprism 特定的键写入 `server.properties`，置于 `aprism.` 命名空间下（例如 `aprism.mod-directory=./aprism/mods`）。Aprism BDS loader 在启动时读取这些键。
...
4. 验证归档内的 Aprism 清单签名（`.aje`/`.abe` 归档包含一个由 Aprism mod 签名密钥签名的 `aprism.sig` 文件）。
5. 将已验证的文件移动到目标目录：
   - JE：`.minecraft/mods/<id>-<version>.aje`
   - BE 客户端：`games/com.mojang/behavior_packs/<id>/`（解包后）或 Aprism 原生 mod 目录。
...
更新渠道：`stable`、`beta`、`nightly`。每个渠道映射到一个 GitHub Releases 标签模式（`v*`、`v*-beta.*`、`v*-nightly.*`）。
...
BE 客户端全局状态位于 `games/com.mojang/aprism/` 下：

- `aprism/installed.json` - Aprism 核心版本、安装时间戳。
- `aprism/mods/<id>/` - 解包后的 `.abe` mod 内容。
...
- 未签名的 Aprism mod（没有有效 `aprism.sig` 的 `.aje`/`.abe`）默认不加载。选择加入的允许列表是每实例的，绝非全局，因此恶意 mod 无法通过在一个实例中被允许而获得提权。
