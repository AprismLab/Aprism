...
3. `quilt.mod.json`（Quilt 兼容性回退；缺失时回退到内嵌的 `fabric.mod.json`）
4. `META-INF/neoforge.mods.toml`（NeoForge 兼容性回退）
5. 带 `loaderVersion` 启发式的 `META-INF/mods.toml`（Forge 兼容性回退）
6. `litemod.json`（LiteLoader 兼容性回退）
...
在 Windows 上，Aprism 在 `Minecraft.Windows.exe` 旁发布一个代理 `version.dll`。该代理将每一个合法导出转发给系统 `version.dll`，并在其 `DllMain` 中加载 `Aprism.dll` 并调用其初始化器。当代理 DLL 方案被杀毒软件或 WDAC 阻断时，存在一条 ExLoader / 手动映射（manual map）回退路径；该回退需要提权权限且仅在用户显式选择时启用（见文档 3 第 4.1 节）。初始化器执行版本检测、签名 DB 选择、钩子安装、模组注册器初始化，最后扫描 `com.mojang/aprism_mods/` 以查找 `.abe` 模组。
...
在 iOS 上，用户的 Mach-O 通过 `insert_dylib` 被打补丁，添加一条指向 `Aprism.dylib` 的 `LC_LOAD_DYLIB` 加载命令。启动时 `dyld` 将 Aprism 与游戏一同加载；Aprism 初始化器运行并通过 Dobby（备选 ElleKit）安装钩子。
...
钩子安装完成后，Aprism 扫描 `aprism_mods/` 以查找 `.abe` 归档。每个归档的清单声明其类型：原生或脚本。原生模组随附与宿主平台匹配的 `.so`、`.dll` 或 `.dylib`；Aprism 对该库执行 `dlopen` 并解析其入口符号（默认 `aprism_mod_load`，见文档 8 §6.2：loader 首先调用该符号，随后经由 `aprism_mod_create` 工厂构造 `IAprismMod` 实例）。脚本模组声明一个 `Script API` 入口点；Aprism 将其注册到 Bedrock 脚本引擎，随后由脚本引擎针对 `@minecraft/server` 求值所打包的 JavaScript。
...
- 文档 3，Aprism 启动器适配、下载安装与管理模块开发指南
- 文档 4，Aprism 产品可行性报告
- 文档 5，Aprism 产品技术报告与研究方法
- 文档 7，Aprism 模组包（.aje/.abe）分类、结构与放置位置
- 文档 8，Aprism Mod 开发者指南
