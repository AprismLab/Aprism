...
| NeoForge-Support.aep | `N` | `neoforge.mods.toml` | `@Mod` 注解，`IEventBus` | `neoforge-mods/` |
| Forge-Support.aep | `Fo` | `mods.toml` | `@Mod` 注解，`@SubscribeEvent` | `forge-mods/` |
...
当一个 loader 专属模组未携带 `aprism.manifest.json` 发布时，扩展的 loader 运行时自动发现其原生清单（`fabric.mod.json`、`neoforge.mods.toml`、`mods.toml` 或 `litemod.json`），并合成一个 Aprism 清单。合成过程是有损的：仅填充具有明确 Aprism 对等项的字段。需要 Aprism 专属功能（跨版本转换、兼容性组声明）的模组必须随附显式的 `aprism.manifest.json`，并以 `.aje` 形式打包至 `mods/`。
...
Aprism 版本遵循 `v<Year>.<aprism>-<stability>.<subVer>-<MCEdit>-<MCVer>`。本文档当前目标为 `v26.0-Alpha.1`，意为 2026 年基线、Aprism 主版本 0、Alpha 细分版本 1。Phase（0-9）为内部开发阶段追踪器，不对公显示，仅记录于 FACT.md 会话日志。Alpha 为开发中的细分版本号（Alpha.1 至 Alpha.9）；不计划开设 Beta。正式发布使用 `PreRelease.N` 然后 `Release`。示例产物：`Aprism-v26.0-Alpha.1-JE-1.21.4`。
