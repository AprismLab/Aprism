...
## pre-26.1 绑定策略（v26.7-Alpha.7）- DEC-PRE261

决策 DEC-PRE261：内容/命令/输入/网络绑定要求 NO_REMAP profile（MC 26.1+，
无混淆官方名）。在 REMAPPED profile 上绑定器以 PROFILE_UNSUPPORTED fail-closed
拒绝并输出日志提示。理由：把 Mojang 官方名跨映射到运行时混淆名需要 Mojang 官方
映射经 intermediary 表链式解析——这是每版本资产加解析工作流，应独立成线
（v26.8 候选）而非仓促附加。优先目标按内部版本表保持 26.x 优先。

## 真实 pre-26.1 映射资产（v26.8-Alpha.8）

REMAPPED profile 现接受两个独立 agent 输入：mappings 指向 Fabric
Intermediary tiny-v2（字节码重映射）；officialMappings 指向 Mojang client.txt
（官方名到运行时名的反射翻译）。两者刻意分离。运行时保存官方映射并传给内容
绑定器，关闭时清理。真实 1.21.4 client.txt 验证：312ms 解析 8857 类，
BuiltInRegistries -> mb、ITEM -> g、BLOCK -> e、Registry.register -> a、
Item.Properties.stacksTo -> a；Identifier 按预期透传。
