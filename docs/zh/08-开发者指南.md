...
|---|---|---|---|
| JDK | Java 25（Temurin/OpenJDK） | 1.16.5 使用 Java 8/11；1.18-1.20.4 使用 Java 17；1.20.5-1.21.11 使用 Java 21（按 Mojang 官方基线） | 编译运行时 |
| Gradle | 9.x | 8.x | 构建编排 |
...
`aprism-packaging` 插件消费 `aprism/` 源集的 `shadowJar` 输出，并将其重新打包为文档 7 定义的 `.aje` 或 `.abe` 归档格式。它不会重新编译或重新映射任何内容；它只负责组装。`.aje` 是 Aprism 原生包：其中只包含 Aprism 原生 jar（独占使用 Aprism API），不包含任何 loader 专属 jar 或按 loader 划分的子目录；loader 适配源集（`fabric/`、`neoforge/`）的产物作为独立的普通模组 jar 分发至对应 loader 的目录，不进入 `.aje`（见文档 7 §3）。

### 8.1 插件所做工作

1. 从配置的 `manifestFile` 属性解析 manifest 文件。
2. 收集 `aprism/` 源集的 shadowJar 产物作为主模组 jar `<modid>.jar`（独占使用 Aprism 原生 API）。
...
主 jar 命名为 `<modid>.jar`，与 manifest 的 `id` 一致。loader 适配源集（`fabric/`、`neoforge/`）的产物**不会**进入 `.aje`：它们作为独立的 loader 原生 jar 单独构建与分发，放置于对应 loader 的模组目录（`fabric-mods/`、`neoforge-mods/`，见文档 7 §6）。`.aje` 仅供 Aprism 原生加载器消费。
...
校验：manifest 位于 ZIP 根目录；manifest 的 `id` 与归档基名匹配；manifest 的 `version` 与归档版本段匹配；主 jar 命名为 `<modid>.jar` 且位于 ZIP 根目录；不存在按 loader 划分的子目录或 loader 专属 jar（`.aje` 仅含 Aprism 原生内容）；manifest 中引用的 mixin 配置存在于 `mixins/` 下。
...
### 10.5 从既有格式转换（aprism convert）

`aprism convert` CLI 子命令是规范的格式转换工具，随 Aprism 一并分发。它将既有格式的模组/包转换为 Aprism 包格式：
...
它接受 `--in <path>`、`--out <path>`、`--format {aje,abe}`（默认由输入类型推断），以及可选的 `--sign` 以产出经 cosign 签名的输出。转换是确定性的：相同输入产出相同输出字节（除 ZIP 条目时间戳被置零外）。转换规则与产物结构的逐条映射见文档 7 第 11 节。该命令面向迁移既有模组的作者；Aprism 原生模组应直接按本指南构建，无需转换。
...
Hook 安装使用 libhat 进行签名扫描，并使用 MinHook（Windows）、ShadowHook（Android）或 Dobby（iOS，备选 ElleKit）进行 detour。Aprism BE API 将后端封装在统一的 `Hook` 接口之后。
